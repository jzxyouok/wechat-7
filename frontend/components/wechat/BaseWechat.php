...
abstract class BaseWechat extends Component
{
    ...
    protected function http($url, $options = [])
    {
        ...
        curl_close($curl);
        $content_type=explode('/', $status['content_type']);
        if(isset($status['http_code']) && $status['http_code'] == 200 && !json_decode($content)){
            return !empty($content) ? $content: false; // 返回图片时不解析json字符串
        }else if(isset($status['http_code']) && $status['http_code'] == 200 && $content_type[0] != 'image'){

...
class MpWechat extends BaseWechat
{
    ...
    {
        $result = $this->httpRaw(self::WECHAT_MATERIAL_GET_PREFIX, [
            'media_id' => $mediaId
        ], [
            'access_token' => $this->getAccessToken()
        ]);
     ...
     * 批量获取用户基本信息
     */
    const WECHAT_USERS_INFO_GET = '/cgi-bin/user/info/batchget';
    /**
     * 获取用户基本信息(UnionID机制)
     * @param $openId
     * @param string $lang
     ...
     * @throws \yii\web\HttpException
     */
    public function getUsersInfo(array $user_list)
    {
        $result = $this->httpRaw(self::WECHAT_USERS_INFO_GET, [
            'user_list' => $user_list
        ], [
            'access_token' => $this->getAccessToken()
        ]);
     ...
     * 个性化菜单接口
     */
    const WECHAT_MENU_CREATE_DIY_PREFIX = '/cgi-bin/menu/addconditional';
    /**
     * 创建个性化菜单
     * @param array $menuArr 结构字符串
     * @param array $matchruleArr 结构字符串
     * ~~~
     *$menuArr=[
     *      [
     *           'type' => 'click',
     ...
     *         "language"=>"zh_CN"
     *         ]
     *  $this->createDiyMenu($menuArr,$matchruleArr);
     * ~~~
     * @return bool
     */
    public function createDiyMenu(array $buttons,array $matchrule)
    {
        $result = $this->httpRaw(self::WECHAT_MENU_CREATE_DIY_PREFIX,[
            'button' => $buttons,
            'matchrule' => $matchrule
        ],[
             'access_token' => $this->getAccessToken()
        ]);
        return isset($result['menuid']);
    }

    /**
     * 个性化菜单删除接口(删除菜单)
     */
    const WECHAT_MENU_DELETE_DIY_PREFIX = '/cgi-bin/menu/delconditional';
    /**
     * 个性化菜单删除接口(删除菜单)
     * @param array $menuid 结构字符串
     *   $menuid=[
     *       "menuid"=>"403905397"
     *   ];
     *
     *   $this->deleteDiyMenu($menuid); 
     * @return bool
     */
    public function deleteDiyMenu(array $menuid)
    {
        $result = $this->httpRaw(self::WECHAT_MENU_DELETE_DIY_PREFIX,[
            'menuid' => $menuid,
        ],[
             'access_token' => $this->getAccessToken()
        ]);
     ...
     * 测试个性化菜单匹配结果
     */
    const WECHAT_MENU_MATCH_DIY_PREFIX = '/cgi-bin/menu/trymatch';
    /**
     * 测试个性化菜单匹配结果
     * @param array $user_id 结构字符串
     *   $user_id=[
     *       "user_id"=>"gardenpop"
     *   ];
     *
     *   $this->matchDiyMenu($user_id); 
     * @return bool
     */
    public function matchDiyMenu(array $user_id)
    {
        $result = $this->httpRaw(self::WECHAT_MENU_MATCH_DIY_PREFIX,[
            'user_id' => $user_id,
        ],[
             'access_token' => $this->getAccessToken()
        ]);
